...
module ConfigurationAPI =
    ...
    let setGitUser (arcConfiguration : ArcConfiguration) (configurationArgs : Map<string,Argument>) =
        ...
        let nameOption = 
            ...
            | Some name -> 
                log.Trace("TRACE: Retrieved user name from given argument.")
                Some name
            | None -> 
                log.Trace("TRACE: Could not retrieve user name from argument. Try to retrieve it from arc config.")
                GeneralConfiguration.tryGetGitName arcConfiguration
            
        let emailOption = 
            match tryGetFieldValueByName "Email" configurationArgs with
            | Some name -> 
                log.Trace("TRACE: Retrieved user email from given argument.")
                Some name
            | None -> 
                log.Trace("TRACE: Could not retrieve user email from argument. Try to retrieve it from arc config.")
        ...
        | Some name, Some email ->
            ...
            let setLocal() =
                ...
                if GitHelper.setLocalName workDir name |> not then
                    log.Error($"ERROR: User name could not be added to local git config in {workDir}.")
                if GitHelper.setLocalEmail workDir email |> not then
                    log.Error($"ERROR: User email could not be added to local git config in {workDir}.")

            let setGlobal() =
                if GitHelper.setGlobalName name |> not then
                    log.Error("ERROR: User name could not be added to global git config.")
                if GitHelper.setGlobalEmail email |> not then
                    log.Error("ERROR: User email could not be added to global git config.")
        ...
        | _, _ -> 
            log.Error("ERROR: Git user metadata neither present in the arc config nor given by argument. Consider first running \"arc auth\", setting the user name and email by running \"arc config set\" or rerunning this command and specifying both name and email.")