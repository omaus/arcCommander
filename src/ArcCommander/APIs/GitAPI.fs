...
module GitAPI =
    ...
    let authenticate (arcConfiguration : ArcConfiguration) =
        ...
        | Ok token -> 
            ...
            log.Trace($"TRACE: Try transfer git user metadata to global arcCommander config")
            ...
            | Some globalConfigPath ->
                ...
                log.Trace($"TRACE: Successfully transferred git user metadata to global arcCommander config")
            | None ->
